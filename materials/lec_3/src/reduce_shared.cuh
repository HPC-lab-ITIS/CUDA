__global__ void reduce_shared(float *in, int n, int m)
{
    ...
    {
        if( ( threadIdx.x % (2*step) == 0) && (threadIdx.x + step < blockDim.x) ) 
...
float test_reduce_shared(std::vector<float> &a, const int n, profiler &prof)
{
    count++;
    ...
    prof.tic("gpu reduction with shared memory");
    for(auto ii = 0; ii < test_runs; ++ii)
    {
        ...
        {
            reduce_shared<<<i, threads>>>(a_dev, first, second);
        ...
        reduce_shared<<<1, leftover>>>(a_dev, first, second);
    ...
    prof.toc("gpu reduction with shared memory");
