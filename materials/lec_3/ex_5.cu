...
__global__ void reduce_6(double* in_data, double* out_data, int size)
{
...
int main(int argc, char **argv)
{
    ...
    prof.tic("reduction gpu 6");
    for (j = n, i = 0; j >= block_size; j = ceil(j, 2*block_size), i ^= 1)
    {
        dim3 threads(block_size, 1, 1);
        dim3 grid(ceil(j, 2*block_size), 1, 1);

        reduce_6<<<grid, threads>>>(a_dev[i], a_dev[i^1], j);
