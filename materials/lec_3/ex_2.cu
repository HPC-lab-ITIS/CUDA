...
__global__ void reduce_3(double* in, int n, int m)
{
    ...
    for(int step = 1; step < blockDim.x; step *= 2)
    {
        ...
        if( ( index < blockDim.x ) && ( index + step < blockDim.x ) )
...
int main(int argc, char **argv)
{
    ...
    auto i2 = n;
    auto l_b = 0;
    prof.tic("reduction gpu 3");
    cudaThreadSynchronize();
    for(auto i = blocks; i > 1; i /= threads)
    {
        reduce_3<<<i, threads>>>(a_dev, i1, i2);
    ...
    reduce_3<<<1, l_b>>>(a_dev, i1, i2);
    cudaThreadSynchronize();
    prof.toc("reduction gpu 3");
