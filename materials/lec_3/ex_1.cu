...
__global__ void reduce_2(double* in, int n, int m)
{
...
int main(int argc, char **argv)
{
    ...
    auto i2 = n;
    auto l_b = 0;
    prof.tic("reduction gpu 2");
    cudaThreadSynchronize();
    for(auto i = blocks; i > 1; i /= threads)
    {
        reduce_2<<<i, threads>>>(a_dev, i1, i2);
    ...
    reduce_2<<<1, l_b>>>(a_dev, i1, i2);
    cudaThreadSynchronize();
    prof.toc("reduction gpu 2");
