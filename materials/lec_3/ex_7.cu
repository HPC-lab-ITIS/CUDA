...
__global__ void scan( double *in_data, double *out_data, int n )
{
    ...
    temp[tid] = in_data[tid + 2 * blockIdx.x * blockDim.x];
    temp[tid + block_size] = in_data[tid + 2 * blockIdx.x * blockDim.x + block_size];
    ...
    out_data[2 * tid + 2 * blockIdx.x * blockDim.x] = temp[2 * tid];
    out_data[2 * tid + 1 + 2 * blockIdx.x * blockDim.x] = temp[2 * tid + 1];
}

__global__ void modify(double *data, double mod, int i)
{
    double tmp = mod + data[i - 1];

    __syncthreads();

    data[threadIdx.x + i] += tmp;
    data[threadIdx.x + block_size + i] += tmp;
}
...
int main(int argc, char **argv)
{
    ...
    auto n = n_par * block_size* block_size;
    auto chunk = 2 * block_size;
    ...
    prof.tic("gpu scan");
    scan<<<n / chunk, block_size>>>(in_dev, out_dev, chunk);
    ...
    for(auto i = chunk; i < n; i += chunk)
    {
        modify<<<1, block_size>>>(out_dev, a[i - 1], i);
        cuerr = cudaGetLastError();
        if (cuerr != cudaSuccess)
        {
            std::cout << "Cannot launch CUDA kernel 2 " << cudaGetErrorString(cuerr) << std::endl;
            return 1;
        }
        ...
        if (cuerr != cudaSuccess)
        {
            std::cout << "Cannot synchronize CUDA kernel 2 " << cudaGetErrorString(cuerr) << std::endl;
            return 1;
        }
    }
